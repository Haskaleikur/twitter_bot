...
def streaming_like_follow (topics)
...
streaming_like_follow("#bonjour_monde")
...
client.search("#bonjour_monde", result_type: "recent").take(3).each do |tweets|
client.favorite(tweets)
end

5.times do 
client.update("#bonjour_monde @the_hacking_pro Salut #{journalist.sample(1)}")
end
